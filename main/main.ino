    ...
    void parseBluetoothData() {
        if (receivedData.startsWith("kp:")) {
            ...
            SerialBT.println("Relay ON command received.");
        } else if (receivedData == "sc") {
            digitalWrite(RELAY_PIN, LOW); // Turn relay off
            Serial.println("Relay OFF command received.");
            SerialBT.println("Relay OFF command received.");
        } else {
            parseTargetLocations(receivedData);
        }
    }

    // Function to send current PID values via Bluetooth
    void sendPIDValuesViaBluetooth() {
        SerialBT.printf("Current PID values -> Kp: %.2f, Ki: %.2f, Kd: %.2f\n", Kp, Ki, Kd);
        Serial.printf("Current PID values -> Kp: %.2f, Ki: %.2f, Kd: %.2f\n", Kp, Ki, Kd);
    }
